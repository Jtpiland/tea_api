...
RSpec.describe 'Add subscription to customer request' do
  describe 'POST /api/v1/customer_subscriptions' do
    it 'can create on the customer subscription table' do
      ...
      post "/api/v1/customers/#{customer.id}/customer_subscriptions", params: { customer_id: customer.id, subscription_id: subscription.id, status: 'active' }
      ...
      expect(customer.subscriptions.first.status).to eq('available')
