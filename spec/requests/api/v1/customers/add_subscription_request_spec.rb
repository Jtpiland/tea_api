...
RSpec.describe 'Add subscription to customer request' do
  describe 'POST /api/v1/customer_subscriptions' do
    it 'can create on the customer subscription table' do
      ...
      post "/api/v1/customer_subscriptions", params: { customer_id: customer.id, subscription_id: subscription.id }
      ...
      expect(customer.subscriptions.first.frequency).to eq('once a month')
    end
  end
end
